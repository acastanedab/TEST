CREATE   PROCEDURE [MNT].[USP_COMBINACION_PARAMETRO_SEL]
(
	...
	@PageNo							INT = 1,
	@PageSize						INT = -1,
	@SortColumn						NVARCHAR(100) = 'CODIGO_COMBINACION_PARAMETRO',
	@SortOrder						NVARCHAR(4)='ASC'
)
AS
/*******************************************************
-- Nombre: MNT.USP_COMBINACION_PARAMETRO_SEL
...
BEGIN 
		...
		SET @lLAStRec = ( @lPageNbr * @lPageSize + 1 );
			...
			WITH CTE_Results
			AS (
				SELECT ROW_NUMBER() OVER (ORDER BY CP.ORDEN_COMBINACION ASC) AS RowNumber,
				Count(*) over () AS RowsTotal,
				Count(*) over () AS RowId
				,CP.CODIGO_COMBINACION_PARAMETRO	AS CodigoCombinacionParametro
			...
			SELECT 
			RowNumber, RowsTotal, RowId, CodigoCombinacionParametro, CodigoCombinacion, CodigoUnidadNegocio, 
			CodigoParametro, Codigo, NombreParametro, TipoParametro, OrdenCombinacion, ImporteFlete, Valor, 
			DescripcionValor,EstadoRegistro, DescripcionEstadoRegistro, Observacion, UsuarioCreacion, 
			FechaCreacion, TerminalCreacion, UsuarioModificacion, FechaModificacion, TerminalModificacion
			FROM CTE_Results
			WHERE @PageSize < 0 OR (RowNumber > @lFirstRec AND RowNumber < @lLAStRec)
			ORDER BY RowNumber ASC;	
END
GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'Obtiene el listado de los registros de la tabla [MNT].[COMBINACION_PARAMETRO].', @level0type = N'SCHEMA', @level0name = N'MNT', @level1type = N'PROCEDURE', @level1name = N'USP_COMBINACION_PARAMETRO_SEL';
